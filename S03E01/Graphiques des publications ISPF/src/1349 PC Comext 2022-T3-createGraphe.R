...
g1 <- ggplot() + 
  geom_bar(data=grapheImpExp[variable=="Exportations locales"], 
           mapping = aes(x = Date, y = value * 30000/1500, fill="Exportations locales"), stat = "identity" )+ 
  ...
  scale_y_continuous(name = expression("Importations civiles"), 
                    ...
                    limits=c(0,30000),
                   sec.axis = sec_axis(~ . * 1500 / 30000 , name = "Exportations locales",labels = scales::number_format(accuracy = 1)))+
...
g2 <- grid.arrange(g05,g02,g04,g06,g09,g08,ncol=3, nrow =2)


saveGrapheFiles(1, largeurCM = 18, hauteurCM = 4.5)
saveGrapheFiles(2, largeurCM = 18, hauteurCM = 11)
