...
g1 <- ggplot() + 
  ...
  scale_y_continuous(name = expression("Importations civiles"), 
                    ...
                    limits=c(0,40000),
                   sec.axis = sec_axis(~ . * 2000 / 40000 , name = "Exportations locales",labels = scales::number_format(accuracy = 1)))+
...
g4 <- plotImpExp("Voitures de tourisme", "Voitures de tourisme", "Millions de F.CFP", Diviseur = 10^6)
g5 <- plotImpExp("Ciments", "Ciments", "Kilotonnes", Diviseur = 10^6)
g6 <- plotImpExp("Perles brutes", "Perles brutes", "kg")
...
saveGrapheFiles(12, largeurCM = 18, hauteurCM = 12)
