...
g1 <- ggplot(graphe1,  aes(x = Annee, y = Nombre,  fill=Type, colour=Type)) +
  ...
  geom_label(data=dfLegendGraphe1,
             ...
             fill=custom.col)+
  geom_label_repel(data=graphe1[Annee==2019],
...
g2 <- ggplot(graphe2,aes(x = Age,y=effectif)) +
  geom_bar(data = graphe2[Annee == 2019], stat = "identity", aes(fill = factor(Sexe)),alpha=0.8, width=0.8) +
  geom_line(data = graphe2[Annee =="1999" & Sexe=="Homme"], stat = "identity",aes(linetype=factor(Annee), group=1),size=0.5) +
  geom_line(data = graphe2[Annee =="1999" & Sexe=="Femme"], aes(linetype=factor(Annee), group=1), stat = "identity",size=0.5) +
  coord_flip() + 
...
g4 <- ggplot(graphe4) +
  ...
  geom_label(data=graphe4 %>% group_by(Taux) %>% arrange(desc(Années)) %>% slice(1),
             aes(x = "1995 - 1999", label=Taux, family="Roboto Light"), size=3)+
  ...
  geom_label_repel(data=graphe4[Années=="2015 - 2019"],
