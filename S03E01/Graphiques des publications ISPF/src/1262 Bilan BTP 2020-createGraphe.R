...
g2 <- ggplot(graphe2,aes(x = Trimestre,y=value, color=variable, group=1)) +
  geom_line()+
...
graphe3[,value:=as.integer(value/10^9)]
...
g3 <- ggplot(graphe3,aes(x = Année,y=value, color=variable)) +
  ...
  scale_colour_manual(values=ispfPalette)+
  theme_ispf()+
  ylab("")+  ylab("Milliards de F.CFP")+
  theme(legend.position = "bottom")
g3


saveGrapheFiles(1, largeurCM = 9, hauteurCM = 4)
saveGrapheFiles(2, largeurCM = 9, hauteurCM = 3)
saveGrapheFiles(3, largeurCM = 9, hauteurCM = 4.9)
