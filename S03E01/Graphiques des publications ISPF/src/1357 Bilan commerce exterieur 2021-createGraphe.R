...
g1 <- ggplot(graphe1,aes(x = Année,y=value, color=variable)) +
  geom_line()+
  scale_x_continuous(labels = scales::number_format(accuracy = 1, big.mark = ''), breaks = seq(2010,2021,2))+
...
colnames(graphe2) <- c("Région", "2020", "2021")
graphe2 <- melt(graphe2,id.vars="Région")
graphe2[,Région:=factor(Région, levels=rev(c("France","Chine","Etats-Unis d'Amérique","Corée du Sud","Nouvelle-Zélande")))]
...
g3 <- ggplot() + 
  geom_bar(data=graphe3[variable!="Perles brutes (volume)"], 
           mapping = aes(x = Année, y = value, fill=variable), stat = "identity")+ 
   geom_line(data=graphe3[variable=="Perles brutes (volume)"],
            mapping = aes(x = Année, y = value*500, colour=variable), size=1, breaks = seq(2015,2021,1))+
...
colnames(graphe5) <- c("Région", "2020","2021")
graphe5 <- melt(graphe5,id = "Région")
graphe5[,Région:=factor(Région, levels=c("Chine","Hong-Kong","Japon","France","Etats-Unis d'Amérique"))]
r <- graphe5[,Région]
graphe5[,Région:=factor(Région, levels=r, ordered=T)]
