...
g2 <- ggplot(arrange(graphe2,desc(Creations)) ,aes(x = reorder(ZoneGeo, Creations),y=Creations)) +
  ...
  geom_text(aes(label=scales::number(Creations)),  position = position_stack(vjust = .5), color="black", size=2)+
  scale_y_continuous(labels = scales::number_format(accuracy = 1))+
  coord_flip()+
  scale_fill_manual(values=ispfPalette)+
  scale_colour_manual(values=ispfPalette)+
...
g3 <- ggplot(graphe3,aes(x = variable,y=value, fill=Secteur, colour=Secteur)) +
  ...
  ylab("")+  xlab("")+
  theme(legend.position = "bottom",legend.direction = "horizontal", legend.text=element_text(size=rel(0.8)))
g3
